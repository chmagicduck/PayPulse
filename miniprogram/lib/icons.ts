/**
 * icons.ts — Lucide SVG 图标工具
 *
 * 使用 Base64 内联 SVG 实现矢量图标。
 * 调用 icon(name, color?, size?) 获取 data URI，可直接用于 <image src>。
 */
...
function buildSvg(name: string, color: string, size: number): string {
...
/**
 * 获取图标的 base64 data URI，可直接用于 <image src="{{iconUri}}">
 */
export function icon(name: string, color: string = '#000000', size: number = 24): string {
  const svg = buildSvg(name, color, size)
  if (!svg) return ''
